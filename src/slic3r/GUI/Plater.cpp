...
struct Plater::priv
{
    ...
    void export_stl_obj(bool extended, bool selection_only);
...
//        view3D_canvas->Bind(EVT_GLTOOLBAR_DELETE_ALL, [q](SimpleEvent&) { q->reset_with_confirm(); });
    ...
    if (wxGetApp().is_editor()) {
        ...
        this->q->Bind(EVT_EXPORT_GCODE_NOTIFICAION_CLICKED, [this](ExportGcodeNotificationClickedEvent&) { this->q->export_stl_obj(true); });
...
void Plater::priv::show_action_buttons(const bool ready_to_slice_) const
{
    ...
    if (get_config_bool("background_processing"))
    {
		...
		if (sidebar->show_reslice(false) |
            ...
            sidebar->show_export_stl(true)) // Added the Export STL button condition
            sidebar->Layout();
    }
    else
    {
        ...
        if (sidebar->show_reslice(ready_to_slice) |
            ...
            sidebar->show_export_stl(!ready_to_slice)) // Added the Export STL button condition
